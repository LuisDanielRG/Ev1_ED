        ...
        try:
            with sqlite3.connect("TallerMecanico.db") as conn:
                ...
                cursor.execute("SELECT S.Nombre, COUNT(DN.NotaID) AS Cantidad FROM Servicios S INNER JOIN DetalleNotas DN ON S.Clave = DN.ServicioID \
                               ...
                               HAVING Cantidad >= ? ORDER BY Cantidad DESC;", (fecha_inicial, fecha_final, cantidad_servicios))
                ...
                if not servicios_mas_prestados:
                    ...
                    while True:
                        ...
                        if opcion_exportar == 1:
                            archivo_csv = f"ReporteServiciosMasPrestados_{fecha_inicial.strftime('%m_%d_%Y')}_{fecha_final.strftime('%m_%d_%Y')}.csv"
                            ExportarDatos(servicios_mas_prestados, archivo_csv, encabezado=encabezado)
                            print(f"\n[green]*** Información exportada correctamente con el nombre [u]{archivo_csv}[/u] ***[/green]")
                        elif opcion_exportar == 2:
                            archivo_excel = f"ReporteServiciosMasPrestados_{fecha_inicial.strftime('%m_%d_%Y')}_{fecha_final.strftime('%m_%d_%Y')}.xlsx"
        ...
        try:
            with sqlite3.connect("TallerMecanico.db") as conn:
                ...
                cursor.execute("SELECT C.Nombre, COUNT(N.Folio) AS Cantidad FROM Clientes C INNER JOIN Notas N ON C.Clave = N.ClienteID \
                               WHERE N.Fecha BETWEEN ? AND ? GROUP BY C.Nombre HAVING Cantidad >= ? ORDER BY Cantidad DESC;", (fecha_inicial, fecha_final, cantidad_clientes))
