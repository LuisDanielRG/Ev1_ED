    ...
    def ServiciosMasPrestados():
        try:
            with sqlite3.connect("TallerMecanico.db") as conn:
                ...
                cursor.execute("SELECT S.Nombre, COUNT(*) AS Cantidad FROM Servicios S \
                                INNER JOIN DetallesServicio DS ON S.ID = DS.ServicioID \
                                INNER JOIN Notas N ON DS.NotaID = N.Folio \
                                WHERE N.Fecha BETWEEN ? AND ? \
                                GROUP BY S.Nombre \
                                ORDER BY Cantidad DESC \
                                LIMIT ?", (fecha_inicial, fecha_final, cantidad_servicios))
                servicios_mas_prestados = cursor.fetchall()

                if not servicios_mas_prestados:
                    print("[red]*** No se encontraron servicios que coincidan con los datos ingresados. ***[/red]")
                else:
                    encabezado = ["NOMBRE DEL SERVICIO", "CANTIDAD DE VECES"]
                    print(f"\n[bold light_pink1]{('SERVICIOS MÁS PRESTADOS').center(75)}[/bold light_pink1]")
